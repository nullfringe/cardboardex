...
Coverage is intentionally incomplete. Vintage Japanese TCGdex records may identify exact catalog cards and standard/no-rarity variants without providing native images. Cardboardex can use a verified TCGplayer CDN image when the exact selected Japanese variant exposes one unique product identity; it does not turn TCGdex's catalog-local IDs into purported printed collector numbers, borrow another variant's pricing, or substitute English artwork. Records without exact variant-scoped product identity and ambiguous multi-variant records remain unresolved.
